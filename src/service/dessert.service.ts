...
export class DessertService {
  ...
  public getByName(name: string): Observable<Dessert> {
    ...
    return this.http.get<Dessert>(`${environment.gatewayUrl}/api/core/desserts/getBy?name=${name}`, {
      headers: new HttpHeaders({
        'Authorization': `Bearer ${token}`
      })
    });
  ...
  public findAll(page: number = 0, size: number = 10): Observable<any> {
    ...
    return this.http.get<any>(`${environment.gatewayUrl}/api/core/desserts`, {
      headers: new HttpHeaders({
        'Authorization': `Bearer ${token}`
      }),
      params: {
